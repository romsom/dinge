...
module pin_header_cutout(n, pin_header_y, wire_distance, depth) {
    ...
    for (x = [-wire_distance / 2, wire_distance / 2]) {
...
difference() {
    wire_length = width + 1;
    wire_distance = 5.08;
    translate([0, 0, - base_height / 2 ]) base_shape();
    ...
    rotate([90,0,0]) cylinder(h=width + 1, d=wire_diameter, center=true);
    // reed / wire
    translate([-wire_distance / 2, 0, 0])
