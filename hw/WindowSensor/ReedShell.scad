...
module pin_header_cutout(n, pin_header_y, wire_distance, depth) {
    ...
    for (x = [0, wire_distance]) {
        translate([x, pin_header_y + depth + 0.5, 0])
            rotate([90, 0, 0])
...
difference() {
    ...
    pin_header_cutout(3, pin_header_y, wire_distance, 3);
    ...
    // reed
    reed_right_boundary = (width / 2) - holder_width + width_offset;
    reed_left_boundary = pin_header_y + 2 + solder_width;
    reed_y_center = reed_y_offset + (reed_left_boundary + reed_right_boundary) / 2;
    translate([0, reed_y_center, 0]) reed_shape(reed_diameter, reed_width, 1, 10);
}